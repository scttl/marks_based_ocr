 ...
 * 
 *
 * $Header: /p/learning/cvs/projects/marks_based_ocr/util/popenw.c,v 1.1 2006-11-22 17:22:26 scottl Exp $
 */
...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (tabix < 0) {
	    ...
	    filetab[tabix] = f;
            ...
            /* set the input stream to be line buffered (instead of full) */
            tabix = setvbuf(f, (char*)NULL, _IOLBF, 0);
            if (tabix != 0) {
                mexErrMsgTxt("Problems altering input stream buffering.");
            }
	    /* return the index */
	    if (nlhs > 0) {
		double *pd;
