...
create_diary = true;
if create_diary
    diary_file = [MOCR_PATH, '/results/unlv_BGTposnorm_all.diary'];
    if exist(diary_file)
        delete(diary_file);
    end
    diary(diary_file);
...
%Syms corpora struct
syms_struct_file = [MOCR_PATH, '/data/reuters_pos_syms.mat'];
...
res_base = [MOCR_PATH, '/results/BGTposnorm'];
...
for ii=1:num_docs
    ...
    if run_pos_map
        [order, score] = positional_learn_mappings(Clust, Syms, ...
                         'dist_metric', 'euc', 'weight_proportion', 0);
