...
create_diary = true;
if create_diary
    diary_file = [MOCR_PATH, '/results/unlv_BGTclassifier.diary'];
    if exist(diary_file)
        delete(diary_file);
    end
    diary(diary_file);
...
%Syms corpora struct
syms_struct_file = [MOCR_PATH, '/data/reuters_BGT_syms.mat'];
...
res_base = [MOCR_PATH, '/results/BGTclassifier'];
...
for ii=1:num_docs
    ...
    if run_pos_map
        [order, score] = positional_learn_mappings(Clust, Syms, ...
                         'dist_metric', 'euc', 'only_unique', false, ...
                         'keep_best', 100);
    ...
    if run_word_map
        ...
        map =zeros(length(order),1);
        for jj=1:length(order)
            map(jj) = order{jj}(1);
        end
        fprintf('best position mapping complete: %f\n', toc);
        save(res_datafile, 'Clust', 'Comps', 'Lines', 'order', 'score', 'map');
    end
