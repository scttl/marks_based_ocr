...
if create_diary
    ...
    '/results/unlv_BGTposnorm_15_simweight_truthtiebreak_all.diary'];
    if exist(diary_file)
        delete(diary_file);
    end
    diary(diary_file);
...
res_base = [MOCR_PATH, '/results/BGTposnorm_15_simweight_truthtiebreak'];
    ...
    if run_word_map
        if run_alt_acc
            [map,valid_acc] = word_lookup_map(Clust, Comps, Syms, 'order', ...
               ...
               'calc_valid_acc', true, 'break_ties_via_truth', false);
            ...
            map = word_lookup_map(Clust, Comps, Syms, 'order', order, ...
                  ...
                  'break_ties_via_density', false, 'break_ties_via_truth',true);
        end
        %@@
        %map = cell2mat(order);
        %map = map(:,1);
